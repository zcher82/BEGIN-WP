...
$om_image_3d_hover_obj=new WPBakeryShortCode_Om_Image_3d_Hover(array( 'base' => 'om_image_3d_hover' ));

$ani=$aos_once=$aos_delay=$aos=$title = $source = $image = $custom_src = $onclick = $img_size = $external_img_size=$instruction=$h_color=
$img_link_large = $link = $img_link_target = $css_animation = $style = $external_style = $border_color =$output= '';
$atts = vc_map_get_attributes( $om_image_3d_hover_obj->getShortcode(), $atts );//p($atts);
extract( $atts );
...
switch ( $source ) {
	...
	case 'featured_image':
		...
		if ( preg_match( '/_circle_2$/', $style ) ) {
			...
			$img_size = $om_image_3d_hover_obj->getImageSquareSize( $img_id, $img_size );
		}
...
$class_to_filter = 'wpb_single_image wpb_content_element ' . $om_image_3d_hover_obj->getCSSAnimation( $css_animation );
$css_class = apply_filters( VC_SHORTCODE_CUSTOM_CSS_FILTER_TAG, $class_to_filter, 'om_image_3d_hover', $atts );
...
if ( $link ) {
	...
	if ( ! empty( $a_attrs['class'] ) ) {
        ...
        $output.='<div '.$aos.' class="portfolio"><div class="rowcontrol om-width"><div class="small-12 columns small-centered"><div class="pic grid">';
$output.='<div class="small-12 columns small-centered"><div><ul class="pic-grid-6 h_pb50"><li class="mix project" data-bound="">
                                        <div class="' . esc_attr($wrapperClass) . ' project__card ' . esc_attr( trim( $css_class ) ) . '">
                                            <a ' . vc_stringify_attributes( $a_attrs ) . ' class="' . esc_attr($wrapperClass) . '">
                                                '.$img['thumbnail'].'
                                                <div class="project__detail"  style="border-color: '.esc_attr($h_color).';">
                                                    <h3 class="project__title">'.esc_html($title).'</h3>
                                                    <small class="project__category">'.esc_html($instruction).'</small>
                                                </div>
                                          </a>
                                        </div>
...
$om_img_3d_js='jQuery(document).ready(function($) {
            $(".project").hover3d({
...
wp_add_inline_script( 'onemax-header-init',$om_img_3d_js );