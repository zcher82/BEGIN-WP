...
$values = (array) vc_param_group_parse_atts( $values );//p($values);
$data = array();
//p($colors);
foreach ( $values as $k => $v ) {
	...
	if ( 'custom' === $style ) {
		...
		if($v['color']==='custom'){
                                                if( ! empty( $v['customcolor'] )){
                                                    $color = $v['customcolor'];
                                                      $highlight = vc_colorCreator( $v['customcolor'], - 10 ); //10% darker
                                                } else {
                                                          $color = $base_colors['normal']['grey'];
                                                          $highlight = $base_colors['active']['grey'];
                                                  }
		...
		$highlight = isset( $colors[ $style ]['active'][ $v['color'] ] ) ? $colors[ $style ]['active'][ $v['color'] ] : $v['active']['color'];
                        ...
                        if(empty($v['customtxtcolor']))
                            $v['customtxtcolor']='black';
	$data[] = array(
		'value' => intval( isset( $v['value'] ) ? $v['value'] : 0 ),
		'color' => $color,
		'highlight' => $highlight,
		'label' => isset( $v['title'] ) ? $v['title'] : '',
                                            'txtcolor'=>$v['customtxtcolor'],
	);
}
//p($data);
...
if ( $legend ) {
	foreach ( $data as $v ) {
		...
		$legend_html .= '<li style="color:'.$v['txtcolor'].';"><span style="background-color:' . $color . '"></span>' . $v['label'] . '</li>';
	}
