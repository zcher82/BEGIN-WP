...
if ( !class_exists( 'ReduxFramework_extension_wbc_importer' ) ) {
    ...
    class ReduxFramework_extension_wbc_importer {
                ...
                foreach ( $imports as $import ) {
                    ...
                    if ( $import['type'] == "d" && !empty( $import['name'] ) ) {
                        ...
                        foreach ( $import['files'] as $file ) {
                            ...
                            case 'color-scheme-list.txt':
                            case 'color-scheme-list.json':
                                $this->wbc_import_files['wbc-import-'.$x]['color_scheme_list'] = $file['name'];
                                break;
            ...
            if ( isset( $_REQUEST['type'] ) && $_REQUEST['type'] == "import-demo-content" && array_key_exists( $_REQUEST['demo_import_id'], $this->wbc_import_files ) ) {
                ...
                if ( file_exists( $demo_data_loc.'/'.$import_parts['content_file'] ) && is_file( $demo_data_loc.'/'.$import_parts['content_file'] ) ) {
                    ...
                    if ( !isset( $import_parts['imported'] ) || true === $reimporting ) {
                    	  $source=$this->demo_data_dir.$import_parts['directory'].'/'.$import_parts['color_scheme_list'];
                        $upload_dir = ReduxColorSchemeFunctions::$upload_dir;
                        $destination = $upload_dir.'/OneMax_opt-color-scheme.json';
                        copy($source,$destination);
                        include $this->extension_dir.'inc/init-installer.php';
                        $installer = new Radium_Theme_Demo_Data_Importer( $this, $this->parent );
                    }else {
                        echo esc_html__( "Demo Already Imported", 'onemax' );
                    }
                }
        ...
        function add_importer_section() {
            ...
            $wbc_importer_label = trim( esc_html( apply_filters( 'wbc_importer_label', __( 'Demo Importer', 'onemax' ) ) ) );

            $wbc_importer_label = ( !empty( $wbc_importer_label ) ) ? $wbc_importer_label : __( 'Demo Importer', 'onemax' );

            $this->parent->sections[] = array(
                'id'     => 'wbc_importer_section',
                'title'  => $wbc_importer_label,
                'desc'   => '<p class="description">'. apply_filters( 'wbc_importer_description', esc_html__( 'Works best to import on a new install of WordPress', 'onemax' ) ).'</p>',
                'icon'   => 'el el-magic',
                'fields' => array(
                    array(
