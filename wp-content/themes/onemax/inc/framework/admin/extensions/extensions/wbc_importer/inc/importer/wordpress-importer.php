...
<?php
	...
	function fetch_remote_file( $url, $post ) {
        ...
        //use WP_Http instead
        $request=new WP_Http();
        $http_args=array('stream' => true,'filename' =>$upload['file']);
        $req_obj=$request->get($url,$http_args);
		...
		if ( isset( $req_obj['headers']['content-length'] ) && $filesize != $req_obj['headers']['content-length'] ) {
			@unlink( $upload['file'] );
		...
		if ( isset($req_obj['headers']['x-final-location']) && $req_obj['headers']['x-final-location'] != $url )
			$this->url_remap[$req_obj['headers']['x-final-location']] = $upload['url'];
